...
public : 
	...
	void del (int val); int delhelp (int val, btnode *root); 
...
{ 
	...
	*newnode = new btnode;   
	...
	n -> count = mid;   
	if (k <= MIN) fillnode (val, c, n, k); 
	else fillnode (val, c, *newnode, k - mid);   
	*y = n -> value[n -> count]; 
...
{ 
	if (i == 0) 
	{ 
		if (root -> child [1] -> count > MIN) leftshift (1); 
		else merge (1); 
	} 
	else 
	{ 
...
{ 
	btree b; 
	int arr[ ] = { 9, 12, 5, 35, 78, 120, 3, 25, 27, 34 }; 
	int sz = sizeof(arr) / sizeof(int); 
	for (int i = 0; i < sz; i++) 
		b.insert(arr[i]); 
	cout << "B-tree of order N=2" << endl; 
	b.show(); 
	b.del(12);
	b.del(5);
