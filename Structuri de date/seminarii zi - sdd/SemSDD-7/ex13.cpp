...
void afisInOrdine_SRD_fisier(FILE *f, bynarytreenode* root) 
{
	if (root) 
	{
		afisInOrdine_SRD_fisier(f, root->left);
		fprintf(f, "\n ISBN = %d, price = %5.2f", root->inf->ISBN, root->inf->price);
		afisInOrdine_SRD_fisier(f, root->right);
	}
}
...
void main()
{
	...
	FILE *fisier;
	fisier=fopen("arbore.txt","w");
    afisInOrdine_SRD_fisier(fisier, radacina);
    fclose(fisier);
