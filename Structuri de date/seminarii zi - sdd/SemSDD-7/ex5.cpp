...
//determinarea elementelor minim si maxim din arbore
void minim_maxim(bynarytreenode* root) 
{
	bynarytreenode* min=root;
	while (min->left) min=min->left;
	printf("\nElementul minim:");
	printf("\n Studentul %s are codul %d si varsta %d", min->info->nume, min->info->cod, min->info->varsta);

	bynarytreenode* max=root;
	while (max->right) max=max->right;
	printf("\nElementul maxim:");
	printf("\n Studentul %s are codul %d si varsta %d", max->info->nume, max->info->cod, max->info->varsta);
}
